export const createTables = (db, table) => {
  db.transaction(txn => {
    if (table == 'bms_meter') {
      txn.executeSql(
        'CREATE TABLE IF NOT EXISTS bms_meter(rowID INTEGER PRIMARY KEY AUTOINCREMENT, entity_cd varchar(5) NULL, meter_id varchar(50) NULL, curr_read_date date NULL, read_by varchar(150) NULL, UNIQUE (entity_cd, meter_id, curr_read_date, read_by) ON CONFLICT IGNORE)',
        [],
        (txn, res) => {
          console.log('table bms_meter created successfully');
        },
        error => {
          console.log('error on creating table bms_meter ' + error.message);
        },
      );
      ...
      txn.executeSql(
        ...
        [],
        (txn, res) => {
          console.log('table bms_volume_trx_temp created successfully');
        },
        error => {
          console.log(
      ...
      txn.executeSql(
        ...
        [],
        (txn, res) => {
          console.log('table bms_meter_log created successfully');
        },
        error => {
          console.log('error on creating table bms_meter_log ' + error.message);
        },
      );
    } else {
      txn.executeSql(
        ...
        [],
        (txn, res) => {
          console.log('table list_meter created successfully');
        },
        error => {
          console.log('error on creating table list_meter ' + error.message);
        },
      );
      // txn.executeSql(
