...
export const store_cm_action = (db, LoginReducer) => {
  const storeAjax = async (data, longitude, latitude) => {
    try {
      ...
      const res = await APIService.submitActivityTakenCorrective(uploadData);
      if (res.data.message == 'success') {
        ...
        db.transaction(tx => {
          tx.executeSql(
            ...
            (tx, result) => {
              command.green('update cm_action_tmp id successfully');
            },
            error => {
  ...
  db.transaction(txn => {
    txn.executeSql(
      ...
      (txn, res) => {
        ...
        command.green('select table cm_action_tmp successfully');
      },
      error => {
        command.red('error on select table cm_action_tmp ' + error.message);
      },
    );
  });
};
