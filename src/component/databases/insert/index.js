...
export const inputTables = (db, data, table) => {
  db.transaction(txn => {
    if (table == 'bms_meter') {
      txn.executeSql(
        'INSERT OR IGNORE INTO bms_meter (entity_cd, project_no, meter_id, curr_read_date, read_by) VALUES (?, ?, ?, ?, ?)',
        [
          data.entity_cd,
          data.project_no,
          data.meter_id,
        ...
        (txn, res) => {
          if (res.rowsAffected > 0) {
            // command.log('insert table bms_meter successfully');
          }
        },
        error => {
          command.red('error on insert table bms_meter ' + error.message);
        },
      );
    } else {
      txn.executeSql(
        ...
        (txn, res) => {
          if (res.rowsAffected > 0) {
            // command.log('insert table list_meter successfully');
          }
        },
        error => {
          command.red('error on insert table list_meter ' + error.message);
        },
      );
    }
  });
  ...
  db.transaction(txn => {
    if (table == 'bms_meter_log') {
      txn.executeSql(
        ...
        (txn, res) => {
          if (res.rowsAffected > 0) {
            command.green('insert table bms_meter_log successfully');
          }
        },
        error => {
          command.red('error on insert table bms_meter_log ' + error.message);
          command.red('error', 'Submit meter log recording failed');
        },
      );
    }
  });
