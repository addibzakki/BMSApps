...
const AdminMeterWritingUpdate = ({navigation}) => {
  const MeterReducer = useSelector(state => state.MeterReducer);
  const LoginReducer = useSelector(state => state.LoginReducer);
  const [currRead, setCurrRead] = useState(MeterReducer.lastReading.last_read.toString());
  const [currReadHigh, setCurrReadHigh] = useState(MeterReducer.lastReading.last_read_high.toString());
  ...
  return (
    <View style={styles.wrapper.page}>
      <Spinner
        visible={loading}
        textContent={'Updating Reading Meter...'}
        textStyle={{color: '#FFF'}}
      />
      <TopHeader
      ...
      <View style={styles.wrapper.subPage}>
        <ScrollView>
          ...
          <ActionButton title="Update" onPress={() => handleNext()} />
          <View style={styles.space(20)} />
        </ScrollView>
      </View>
    </View>
...
export default AdminMeterWritingUpdate;
