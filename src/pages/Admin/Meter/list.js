...
const AdminMeterListHistory = ({navigation, route}) => {
  ...
  const getData = () => {
    let readBy;
    if (
      route.params.read_level == 'Admin' ||
      route.params.read_level == 'Supervisor'
    ) {
      readBy = 'All';
    } else {
      readBy = route.params.read_by;
    }

    const url =
      'https://mmpportal.mmproperty.com/api/get_history_meter/' +
      route.params.type +
      '/' +
      route.params.tenant +
      '/' +
...
export default AdminMeterListHistory;
