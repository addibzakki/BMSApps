...
const AdminDashboard = ({navigation}) => {
      ...
      if (typeof resD !== 'undefined') {
        if (resD.data.show_message == true) {
          Alert.alert('Alert', resD.data.message, [
            {
              text: 'Update',
              onPress: () => Linking.openURL(resD.data.link),
              style: 'default',
            },
          ]);
        } else {
          if (resD.data.password_check == 'notset') {
            setShowModal(true);
          } else {
            changeModal(false, 'Checking..');
            // ? check work area
            if (resD.data.status == 'available') {
              dispatch(setArea(resD.data.area[0], true));
              setLoading(false);
            } else {
              dispatch(clearArea());
              setLoading(false);
            ...
            if (resD.data.get.total_corrective > 0) {
              setNotifHelpdesk(true);
              setValNotifHelpdesk(resD.data.get.total_corrective);
            }
            if (resD.data.get.total_overtime > 0) {
              setNotifOvertime(true);
              setValNotifOvertime(resD.data.get.total_overtime);
            }
            if (resD.data.get.total_spl > 0) {
              setNotifSpl(true);
              setValNotifSpl(resD.data.get.total_spl);
            }
            setListSubmitTicket([...resD.data.get.list]);
          }
  ...
  const checkSPL = async () => {
    if (LoginReducer.form.profile.level == 'Supervisor') {
      ...
      try {
        ...
        const resS = await SPLAPIService.getOutstandingSPL(params);
        if (resS != 'undefined') {
          if (resS.data.count > 0) {
            setShowModalSPL(true);
            setDataSPL(resS.data.data);
  ...
  const changePassword = async () => {
    if (newPassword.trim() == '') {
      ...
      try {
        ...
        if (error.message == 'Network Error') {
          Alert.alert('Jaringan Error', 'Silahkan periksa jaringan anda 3');
        } else {
          Alert.alert('Error', error.message);
          console.log(error);
        }
  ...
  const getPoint = async () => {
    try {
      const resP = await DashboardAPIService.getPoint(
        LoginReducer.form.profile.uid,
      );
      if (resP != 'undefined') {
        setPoint(resP.data.point);
      }
    } catch (error) {
      if (error.message == 'Network Error') {
        Alert.alert('Jaringan Error', 'Silahkan periksa jaringan anda 5');
      } else {
        Alert.alert('Error', error.message);
        console.log(error);
      }
